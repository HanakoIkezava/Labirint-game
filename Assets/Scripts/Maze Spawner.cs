...
public class MazeSpawner : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        Vector3 mazeCenterOffset = new Vector3((width * cellWidth) / 2.0f, 0, (height * cellHeight) / 2.0f);

        for (int x = 0; x < width; x++)
        {
            for (int y = 0; y < height; y++)
            {
                Vector3 cellPosition = new Vector3(x * cellWidth, 0, y * cellHeight) - mazeCenterOffset;
